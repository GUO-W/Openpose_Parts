...
namespace op
{
    PoseModel flagsToPoseModel(const std::string& poseModeString)
    {
        try
        {
	    ...
	    else if (poseModeString == "COCO_headshoulder")
                return PoseModel::COCO_headshoulder;
            ...
            // else
            error("String does not correspond to any model (COCO, COCO_legs,COCO_headshoulder, MPI, MPI_4_layers)",
